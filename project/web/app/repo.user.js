...
exports.add = async (username, email, hash, apikey) => {
    const db = await getDbConnection();
    const sql = `INSERT INTO users (username, email, password, apikey, roles) VALUES (?, ?, ?, ?, ?)`;
    const roles = JSON.stringify(['USER']); 
    return await db.query(sql, [username, email, hash, apikey, roles]);
};
